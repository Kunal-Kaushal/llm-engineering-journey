...
llm = ChatOpenAI(
    ...
    api_key=os.getenv("DEEPSEEK_V4_FLASH"),
    temperature=0,
    timeout=60,
    max_retries=0,
)
...
def fetch_context(question: str) -> list[Document]:
    return _retriever.invoke(question, k=10)
...
def _with_retry(fn, max_retries=10):
    """Call fn(), waiting 65s on 429 to let the rate limit window reset."""
    for attempt in range(max_retries):
        try:
            return fn()
        except Exception as e:
            if "429" in str(e) and attempt < max_retries - 1:
                print(f"Rate limited (attempt {attempt+1}), waiting 65s...")
                time.sleep(65)
            else:
                raise
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    generated_answer, retrieved_docs = _with_retry(lambda: answer_question(test.question))
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    answer_eval = _with_retry(lambda: llm.with_structured_output(AnswerEval).invoke(judge_messages))
